...
def encrypt_with_aes(input_string, password, salt):
    ...
    f = Fernet(key)
    # call the Fernet encrypt method
...
def decrypt_with_aes(encrypted_data, password, salt):
    key = generate_aes_key(password, salt)
    f = Fernet(key)
    # call the Fernet decrypt method
    decrypted_data = f.decrypt(encrypted_data) 
    return decrypted_data.decode('utf-8')

# Prepare Encryption Parameters
salt = b'Tandon' # Encoded as a byte-object
password = "gaw6837@nyu.edu" # Updated to match your exact Gradescope email
...
dns_records = {
    ...
    'nyu.edu.': {
        ...
        # Decoded to a string to prevent the b'' wrapper from corrupting the token during transmission
...
def run_dns_server():
    # Create a UDP socket (SOCK_DGRAM) and bind it to localhost ('127.0.0.1') and standard DNS port (53)
